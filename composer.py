...
def get_pianoroll_from_notes(notes):
    ...
    for i,j in itertools.product(range(notes_rows),range(notes_cols)):
...
def play():
    ...
    print(f"Loading gaussian/pca statistics...")
    ...
    while running:
        ...
        if autosavenow:
            ...
            while True:
                ...
                if audio_time == 0:
                    break
            wave_output: typing.Union[Wave_write,typing.Any] = wave.open(
                'results/history/autosave' + str(autosavenum) + '.wav','w')
        ...
        if blend and blendcycle > 10:
            ...
            if blendstate%2 == 0:
                ...
                cur_controls: ndarray = np.copy(keyframe_controls[int(blendstate / 2)])
                apply_controls()
            elif blendstate%2 == 1:
                for x in range(0,len(current_params)):
                    current_params[x] = (blendfactor * keyframe_params[int(blendstate / 2),x]) + (
                                (1 - blendfactor) * keyframe_params[
                            ((int(blendstate / 2)) + 1) % len(keyframe_paths),x])
                if blend_slerp:
                    magnitude = (blendfactor * keyframe_magnitudes[int(blendstate / 2)]) + (
                                (1 - blendfactor) * keyframe_magnitudes[
                            ((int(blendstate / 2)) + 1) % len(keyframe_paths)])
                    current_params = current_params * ((sum(current_params*current_params)**-0.5) * magnitude)
                for x in range(0,len(cur_controls)):
                    cur_controls[x] = (blendfactor * keyframe_controls[int(blendstate / 2),x]) + (
                                (1 - blendfactor) * keyframe_controls[
                            ((int(blendstate / 2)) + 1) % len(keyframe_paths),x])
                apply_controls()
                needs_update = True
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # QUIT BUTTON HIT
                ...
                if event.key == pygame.K_b:  # KEYDOWN B
                    ...
                    if blend:
                        ...
                        for y in range(blendnum):
                            file_name = input("The file name of the next song to be blended ")
                            if "." not in file_name:
                                file_name = file_name + ".txt"
                            keyframe_paths.append(file_name)
                            fo: typing.TextIO = open("results/history/" + file_name,"r")
                ...
                if event.key == pygame.K_s:  # KEYDOWN S
                    ...
                    if "." not in file_name:
                        file_name = file_name + ".txt"
                    with open("results/history/" + file_name,"w") as text_file:
                        if blend:
                            text_file.write(sub_dir_name + "\n")
                ...
                if event.key == pygame.K_l:  # KEYDOWN L
                    ...
                    if not sub_dir_name == fo.readline()[:-1]:
                        running = False
                        print(f"incompatable with current model")
                        break
                    tempDir = fo.readline()
                ...
                if event.key == pygame.K_o:  # KEYDOWN O
                    ...
                    if not songs_loaded:
                        ...
                        try:
                            y_samples: typing.Union[
                                typing.Union[np.ndarray,typing.Iterable,int,float,tuple,dict],typing.Any] = np.load(
                                'data/interim/samples.npy')
                            y_lengths = np.load('data/interim/lengths.npy')
                            songs_loaded = True
                        except Exception as e:
                            print(
                                f"This functionality is to check if the model training went well by reproducing an original song. "
                                f"The composer could not load samples and lengths from model training. "
                                f"If you have the midi files, the model was trained with, process them by using"
                                f" the preprocess_songs.py to find the requested files in data/interim "
                                f"(Load exception: {0}".format(e))

                    if songs_loaded:
                        # check how well the autoencoder can reconstruct a random song
                        print(f"Random Song Index: " + str(random_song_ix))
                        if is_ae:
                ...
                if event.key == pygame.K_m:  # KEYDOWN M
                    ...
                    midi_utils.samples_to_midi(
                        current_notes,'results/history/' + file_name,note_threshold)
                    audio_pause = False
                ...
                if event.key == pygame.K_w:  # KEYDOWN W
                    ...
                    while True:
                        ...
                        if audio_time == 0:
                            break
                    wave_output = wave.open('results/history/' + file_name + '.wav','w')
                    wave_output.setparams(
                        1,2,sample_rate,0,'NONE','not compressed')
                    wave_output.writeframes(save_audio)
                    wave_output.close()
        ...
        if needs_update:
            if use_pca:
                latent_x = latent_means + \
                    np.dot(current_params * latent_pca_values, latent_pca_vectors)
            else:
...
def loadsongfile(file_name):
    ...
    if "." not in file_name:
        file_name = file_name + ".txt"
    fo = open("results/history/" + file_name,"r")
    print(fo.name)
    if not sub_dir_name == fo.readline()[:-1]:
        running = False
        print("incompatable with current model")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str,
                        help='The folder the model is stored in (e.g. a folder named e and a number located in results/history/).',
                        required=True)
